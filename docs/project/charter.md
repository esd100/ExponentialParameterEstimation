...
*Tissue-level threshold (v0.10).* Two synthetic amplitude vectors are a kernel-level statement; the body is not one myelin-like split. The harness now carries a **tissue and organ dictionary** (`mexp/tissues.py`; §4 Phase 0; §6.1 ground-truth axis): literature-derived compositions — component fractions and time constants, the clinical functional of each (myelin water fraction, luminal water fraction, extracellular fraction, fat fraction …), and a typical acquisition — for brain white and grey matter, spinal cord, skeletal muscle, articular cartilage, prostate peripheral zone, breast, myocardium (a K = 1 null case) and liver on the T2 kernel, with cortical bone and tendon (UTE T2\*), white-matter T1 and liver IVIM carried for the kernels to come. Every entry is provenance-flagged; as of v0.10 all are `RECALLED` and the verification pass is §10 step 8. The clauses above are evaluated at every entry at the reference acquisition and at the entry's own (`scripts/phase0_threshold_tissues.py`, `results/threshold_tissues.md`). First reading, subject to that flag: under a *free* K = 3 model no myelin-type entry is estimable at first-echo SNR 100 — the unpinned 2000 ms component roughly doubles the bound on the myelin water fraction — while with the long component fixed or dropped the white-matter MWF bound is ±0.030 (1σ) at SNR 100 and ±0.010 at SNR 300, with the myelin T2 itself still at 57 % relative SD; the functional is better determined than the parameters, a first sighting of Phase 3's "estimate functionals, not spectra" at the level of the bound. Minor components under ~5 % (grey-matter myelin water, the liver vascular fraction) fail at clinical SNR; well-separated two-component tissues (prostate luminal water, ratio ≈ 8) are estimable, and at their own 64 × 8 ms acquisition the luminal water fraction reaches 5 % relative. The "K = 2 comfortably, K = 3 marginal" figure therefore reads, per tissue: **K = 2 comfortably where the minor component exceeds ~10 % and the ratio exceeds ~3 (spinal cord, prostate, muscle's two visible compartments); marginal for white matter's myelin water at SNR 100; not supportable for minor components under 5 %; and K = 3 nowhere at clinical SNR without constraining the long component.** Revision of that sentence follows the dictionary's verification, not argument.
...
- **Tissue and organ dictionary** *(added in v0.10)*: `mexp/tissues.py` — one entry per (tissue, modality, field): component fractions and time constants with literature ranges, the clinical functional and its threshold, a typical acquisition, sources, and a provenance status (`RECALLED` → `SECONDARY` → `PRIMARY`). It is the ground-truth axis's "literature-derived composition" level (§6.1) and the population over which the §1.2 clauses are evaluated. Thirteen entries seeded 2026-09-07, all `RECALLED`; components below the first echo are dropped and renormalised at evaluation time, so a 3 ms cartilage component is not pretended visible to a 10 ms train. Verification pass: §10 step 8.
...
2. Stand up the Phase 0 harness skeleton with the pluggable-kernel interface. **Done 2026-09-07** (`claude/phase0-harness-status.md`): §6.1 axes as enums, design generators including verified sparse rulers, the kernel ABC with a single abstract method, `t2_cpmg` as the only *registered* kernel (IR, Look-Locker, complex T2\*, T1ρ-dispersion, and b-tensor kernels are exercised against the base class in tests but deliberately not registered, per G2), the k-space-first rule enforced by test, 55 tests passing (v0.9). **Resolved 2026-09-07 (v0.10):** the code lives in the git repository in the connected folder `ExponentialParameterEstimation/mexp-harness/`, with the living documents mirrored under `docs/project/` (§7).
...
8. **Tissue-dictionary verification pass** *(added v0.10)*: open each source named in `mexp/tissues.py`, move the entry's status from `RECALLED` to `SECONDARY` or `PRIMARY` with the paper's own numbers and field strength, and re-run `scripts/phase0_threshold_tissues.py`; log any entry whose estimability class changes in §8. Add entries as modalities' kernels are registered (T1 IR, UTE T2\*, IVIM) and as organs are added (kidney, bone marrow, tumour heterogeneity as a continuous truth). Also open a row in `bibliography.md` for every source touched.
