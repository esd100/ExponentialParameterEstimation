...
\* Stanisz 2005 was read in full through a mirrored PDF; the publisher PDF is now in `literature/` and the row moves to `CHECKED-PRIMARY` when the next session re-reads Table 1 from it (Bojorquez 2017 moved on 2026-09-09). All 57 items of `literature-requests.md` are in `literature/` as of 2026-09-08 (retrieved by Eric); the remaining `CHECKED-ABSTRACT` / `BIBLIO-ONLY` rows above are the next session's reading list (charter §10 step 8).
