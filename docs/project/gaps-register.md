...
*Phase 0 numerical note (2026-09-10; status unchanged):* the CRLB map over the tissue dictionary (`results/crlb_map.md`) puts the joint-σ cost under 2 % for every row whose signal stays above the noise floor across the echo train and at 6–10 % — with the magnitude arm itself 16–37 % above the phase-corrected arm — for the steatotic-liver rows, whose 36 ms water component decays into the floor within a 320 ms window. Item 3's degeneracy manifold therefore has a first physical instance (a short-T2 majority component on a long train), and the analytic work should start from that geometry rather than from the long-T end alone.
...
**Status:** `UNVERIFIED` · **Effort:** S–M · **Payoff:** Medium-high · **Confidence:** 0.6 *(raised to 0.7 in v0.7: the 2026-09-07 literature pass found no such table and found the abdominal multi-component measurements themselves missing; unchanged in v0.8; v0.9 (2026-09-09): the dictionary's priority-1 verification pass read the twelve classic multi-component papers in full and none of them tabulates more than its own tissue — and four of the numbers the project had carried from memory of them were wrong (prostate fraction, ratio and protocol; muscle extracellular fraction; tendon split; WM short T1), which is the case for a curated, sourced table stated by example)*
...
**Future research project (recorded 2026-09-09, Eric's decision: not current focus).** *In vivo multi-component T2 relaxometry of liver, spleen, kidney and pancreas, acquired in the same session as IVIM, so that the T2-derived and diffusion-derived compartment fractions can be compared against each other and against the fast-exchange prediction that the T2 axis mostly cannot see the vascular pool.* It would supply the abdominal rows of this table from measurement, and it is the natural experimental companion of the harness's 'exchange-coupled ground truth' axis (charter §6.1). It is parked here so that it is neither lost nor pursued by accident; nothing in the charter's phases depends on it, and the THEORY labels stay until such a study exists. The pathology axis opened in dictionary v2.3 (2026-09-10): `condition` and `clinical_delta` on every entry and five disease rows on primaries (steatotic liver at PDFF 10 / 25 % — Bydder 2008's water / fat T2 pair with Tang 2013 / 2015's grade calibration; malignant prostate PZ — Sabouri 2017; venous-filled muscle — Araujo 2014; MS-lesion WM — MacKay 1994), 24 entries and 67 component values (32 PRIMARY); the water–fat pass moved the liver, pancreas and kidney PDFF to PRIMARY and found that the one review expected to tabulate organ fat across the body (Hu 2011) does not — another instance of this entry's claim, on the fat axis. The steatotic rows also record, for the table's users, that the T2 axis cannot grade steatosis (ratio 2) and that chemical-shift encoding is the measured tool.
