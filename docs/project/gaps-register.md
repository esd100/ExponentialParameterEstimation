...
*Adjacency note (2026-09-06, from the pass-1 foundations reading; B1 itself not checked, status unchanged; formula aligned 2026-09-07):* the positivity-free baseline any B1 result must beat is now explicit — the McWhirter–Pike / Bertero et al. resolution limit δ(SNR) = exp(π²/arccosh(SNR²)) in the continuum (2.7 at SNR 100), and the harness's measured 3.16 for a 32 × 10 ms train (charter §1.2), with Epstein & Schotland (2008) supplying the closed-form singular functions a sharp-constant statement would be written in. A B1 result that does not improve on δ by a stated factor attributable to positivity is not a result.
...
*Adjacency note (2026-09-06; updated 2026-09-07):* charter §1.2 now states the (SNR, acquisition window, T-range) → informative-direction map, measured by the harness for the T2 kernel and matched to the arccosh continuum form with γ_eff = min(T-range, window), with a falsifiable threshold on the step to components. D3 is the multi-modality, exchange-aware, precision-stated extension of that map; the measured map is its zeroth-order term, not a competitor. Status unchanged.
...
**Status:** `UNVERIFIED` · **Effort:** S–M · **Payoff:** Medium-high · **Confidence:** 0.6 *(raised to 0.7 in v0.7: the 2026-09-07 literature pass found no such table and found the abdominal multi-component measurements themselves missing)*

Method papers each carry their own "typical" compositions (myelin water ~0.1 at ~15 ms, and so on) drawn from a handful of classic studies, but no curated table appears to exist that lists, per tissue and organ, per modality and field strength, the component fractions and time constants *with their literature ranges and sources*, in a form a simulator can load. The charter's tissue dictionary (`mexp/tissue_data.py`, charter v0.10–v0.11) is a first draft of exactly this: version 2 (2026-09-07) has 18 entries (brain WM/GM, CSF, cord, muscle, cartilage, tendon, cortical bone, marrow, myocardium, blood, liver, spleen, kidney cortex/medulla, pancreas, prostate, breast) with water/PD, T1 and T2 at 1.5 and 3 T, ADC, components per modality (T2, T1, IVIM/tensor/standard-model diffusion), a physical-pool theory block, and a provenance status on every value — 7 PRIMARY, 30 SECONDARY, 1 TERTIARY, 6 RECALLED, 10 THEORY component values. The THEORY rows (liver, spleen, kidney, pancreas T2 splits) exist precisely because the literature search found *no* in vivo multi-component T2 study of those organs — which sharpens this entry: the gap is not only the table, it is the measurements the table would need for the abdomen. The pathology axis (one disease variant per organ) is the next revision (charter §10 step 8).
