# Phase 0 harness — status (2026-09-09, revision 6)

Companion to `claude/phase0-findings.md` (revision 2 + addenda through revision 5) and charter v0.13. Short, operational; update when the harness moves.
...
`pip install -e ".[dev]" && pytest` inside `mexp-harness/` (71 tests, ~25 s). Public mirror: `github.com/esd100/ExponentialParameterEstimation` (created and pushed by Eric 2026-09-09; tags `charter-v0.10`–`charter-v0.13`). Licenses: Apache-2.0 (code), CC BY 4.0 (dictionary and documents) — repository root `LICENSE`, `LICENSE-DOCS`. The local Cowork VM has no scipy, so
...
5. **Dictionary verification pass** (charter §10 step 8): **priority 1 done 2026-09-09** — Sabouri 2017 ×2, Whittall 1997, MacKay 1994, Saab 1999, Araujo 2014, Stanisz 2005 (publisher copy), MacMillan 2011, Labadie 2014, Prasloski 2012, Du 2012, Bouhrara 2015 read and applied (dictionary v2.2; `results/threshold_tissues.md` re-run; two class changes in charter §8: prostate at the reference train down, cord under K = 2 up). **Not movable from the PDFs on hand:** the lean-organ PDFF ranges — no water–fat paper among the 57; the nine that would settle them are `literature-requests.md` priority 1b (#58–#66). **Next:** the pathology axis, steatotic liver at PDFF 10 / 25 % first (charter §10 step 8e; provenance flag until Tang 2013 / Yokoo 2018 / Hamilton 2011 are on hand), then the priority-2 bulk-property reads (T1 at 3 T, relative PD). The abdominal THEORY splits stay: measuring them is a recorded future research project (register E3), not current focus.
